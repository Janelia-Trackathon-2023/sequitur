...
def test_io_nodemodel(tmpdir, simple_nodes):
    # write nodes
    path = Path(tmpdir, 'nodes.parquet')
    _write_parquet_api(path, simple_nodes)
    ...
    assert nodes_read == simple_nodes


def test_io_edgemodel(tmpdir, simple_edges):
    # write edges
    path = Path(tmpdir, 'edges.parquet')
    _write_parquet_api(path, simple_edges)
    ...
    assert edges_read == simple_edges


def test_io_graph(tmpdir, simple_nodes, simple_edges):
    # write nodes
    node_path = Path(tmpdir, 'nodes.parquet')
    _write_parquet_api(node_path, simple_nodes)

    # write edges
    edge_path = Path(tmpdir, 'edges.parquet')
    _write_parquet_api(edge_path, simple_edges)

    # read the file back
    graph = read_graph(node_path, edge_path)

    assert graph['nodes'] == simple_nodes
    assert graph['edges'] == simple_edges
